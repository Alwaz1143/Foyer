...
export function useWeather() {
  ...
  const fetchWeather = useCallback(async (loc: typeof location) => {
    if (!loc) return;
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${loc.lat}&longitude=${loc.lon}&current=temperature_2m,apparent_temperature,weather_code,relative_humidity_2m,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min,weather_code&timezone=auto`
      );
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      if (!cancelled) await fetchWeather(location);
    };
    doFetch();
  ...
  }, [location, fetchWeather, retryCount]);

  const retry = useCallback(() => setRetryCount((c) => c + 1), []);

  return { weather, loading, location, error, retry };
}
